...
const userId = functions.config().naver.id
const password = functions.config().naver.password
...
exports.macro = functions.region('asia-northeast1').runWith(runtimeOptions).https.onRequest(async (req, res) => {
  let browser = null
  let page = null
  try {
    ...
    res.send("Firebase Function Compelted!");
  } catch (err) {
    console.error(err)
  } finally {
    if (page !== null) {
